...
class Module_c4g_forum extends Module
{
	...
	/**
	 * 
	 * put all threads of a forum into the jQuery-DataTable
	 * 
	 * @param int $id - ID of the forum
	 * @param boolean $forumTree - TRUE means also recreate the forum tree
	 */
	public function getForumInTable( $id, $forumTree )
	{
		...
		$data['aoColumnDefs'] = array(
		  array('sTitle' => 'key', bVisible => false, bSearchable => false, aTargets => array(0)),
		  array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['THREAD'], sClass => 'c4g_forum_tlist_threadname', sWidth => '50%', aDataSort => array(9,1), aTargets => array(1), c4gMinTableSizeWidths => array(array(tsize=>500,width=>'50%'),array(tsize=>0,width=>''))),
		  array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['LAST_AUTHOR_SHORT'], sClass => 'c4g_forum_tlist_last_author', aDataSort => array(9,2,4), bSearchable => false, aTargets => array(2), c4gMinTableSize => 700),
		  array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['LAST_POST_SHORT'], sClass => 'c4g_forum_tlist_last_post', aDataSort => array(10,4), bSearchable => false, asSorting => array( 'desc', 'asc' ), aTargets => array(3), c4gMinTableSize => 700),
		  array(bVisible=>false, bSearchable => false, aTargets=>array(4)  ),
		  array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['AUTHOR'], sClass => 'c4g_forum_tlist_author', aDataSort => array(9,5,7), bSearchable => false, aTargets => array(5), c4gMinTableSize => 500),
		  array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['CREATED_ON'], sClass => 'c4g_forum_tlist_created', aDataSort => array(10,7), asSorting => array( 'desc', 'asc' ), bSearchable => false, aTargets => array(6), c4gMinTableSize => 500),
		  array(bVisible=>false, bSearchable => false, aTargets=>array(7)),
		  array('sTitle' => '#', sClass => 'c4g_forum_tlist_postcount', asSorting => array( 'desc', 'asc' ), bSearchable => false, aTargets => array(8)),
		  array(bVisible=>false, bSearchable => false, aTargets=>array(9)),
		  array(bVisible=>false, bSearchable => false, aTargets=>array(10)),
		  array(bVisible=>false, bSearchable => false, aTargets=>array(11)),
		  );
		...
		$data['aaSorting'] = array(array(3, 'desc'));
	...
	public function getForumTree($actForum) 
	{
		...
		$children = $this->getForums($this->c4g_forum_startforum, $actForum);		
  		...
  		$treedata = array( 
			...
			fx => array(height => "toggle", duration => 200)   		 	
  		 	);
  		return $treedata; 	
	}
	...
	public function getPostAsHtml( $id )
	{
		...
		$dialogbuttons = array(
  				array( action => 'closedialog:post'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CLOSE'])
  		);
  			
	  	if ($this->helper->checkPermission($posts[0]['forumid'],'newpost')) {
  			array_insert($dialogbuttons, 0, 
  				array(array( action => 'newpost:'.$posts[0]['threadid'].':post'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['NEW_POST']))
  			);
  		}
  		...
  		foreach ($act as $key => $value) {
  			array_insert($dialogbuttons, 0, 
  				array(array( action => $key, type => 'get', text => $value))
  			);
  		}
	...
	public function getThreadAsHtml( $id )
	{
		...
		$dialogbuttons =array(
  				array( action => 'closedialog:thread'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CLOSE'])
  			);
		...
		if (FE_USER_LOGGED_IN) {		
  			if ($this->helper->checkPermission($thread['forumid'],'subscribethread')) {
  				...
  				if ($showButton) {
					...
					array_insert($dialogbuttons, 0, 
		  				array(array( action => 'subscribethreaddialog:'.$id, type => 'get', text => $text ))
		  			);
  				}
  		...
  		if ($this->helper->checkPermission($thread['forumid'],'newpost')) {
  			array_insert($dialogbuttons, 0, 
  				array(array( action => 'newpost:'.$id.':thread'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['NEW_POST']))
  			);
  		}
  		
  		if ($this->helper->checkPermission($thread['forumid'],'movethread')) {
  			array_insert($dialogbuttons, 0, 
  				array(array( action => 'movethreaddialog:'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['MOVE_THREAD']))
  			);
  		}  	
  			
  		if ($this->helper->checkPermission($thread['forumid'],'delthread')) {
  			array_insert($dialogbuttons, 0, 
  				array(array( action => 'delthreaddialog:'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['DEL_THREAD']))
  			);
  		}  		
  		...
  		if ($this->helper->checkPermissionForAction($thread['forumid'],$editAction)) {
  			array_insert($dialogbuttons, 0, 
  				array(array( action => $editAction.':'.$id, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['EDIT_THREAD']))
  			);
  		}  		
	...
	public function generateNewThreadForm($forumId)
	{
		...
		$data =  '<div class="c4gForumNewThread">'.
				 ...
				 '<input name="thread" type="text" class="formdata ui-corner-all" size="80" maxlength="100"></input><br />'.
				 '</div>';
		...
		$return = array(
			...
			dialogdata => $data,
  			dialogbuttons => array(
  				array( action => 'sendthread:'.$forumId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SEND']),
  				array( action => 'previewthread:'.$forumId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['PREVIEW']),
  				array( action => 'cancelthread:'.$forumId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),	
  			dialogoptions => $this->addDefaultDialogOptions(array(
  				title => 
	...
	public function generateNewPostForm($threadId, $parentDialog)
	{
		...
		$data = '<div class="c4gForumNewPost">'.
				...
				'<input name="subject" value="'.$thread['threadname'].'" type="text" class="formdata ui-corner-all" size="80" maxlength="100"></input><br />'.
				'</div>'.
				'<div class="c4gForumNewPostContent">'.
				$GLOBALS['TL_LANG']['C4G_FORUM']['POST'].':<br/>'.
				'<input type="hidden" name="uploadEnv" value="{{env::path}}">'.
				'<input type="hidden" name="uploadPath" value="'.$this->c4g_forum_bbcodes_editor_imguploadpath.'">'.
		...
		$return = array(
			...
			dialogstate => "forum:".$thread['forumid'].";newpost:".$threadId,			
  			dialogbuttons => array(
  				array( action => 'sendpost:'.$threadId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SEND']),
  				array( action => 'previewpost:'.$threadId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['PREVIEW']),
  				array( action => 'cancelpost:'.$threadId.':newpost', type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),	
  			dialogoptions => $this->addDefaultDialogOptions(array(
	...
	public function sendPost( $threadId )
	{
		...
		$result = $this->helper->insertPostIntoDB($threadId, $this->User->id, $this->putVars['subject'],$this->putVars['post'],
	...
	public function previewPost( $threadId, $title )
	{
		...
		$return = array( 
  			...
  			dialogbuttons => array(
  				array( action => 'closedialog:previewpost', type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CLOSE'])
  			),  			
  		);
	...
	public function cancelPost( $threadId, $close )
	{
		...
		$return = array(
			dialogclose => array( "readthread:".$threadId, $close ),
	...
	public function sendThread( $forumId )
	{
		...
		$result = $this->helper->insertThreadIntoDB($forumId,$this->putVars['thread'],$this->User->id,$threaddesc,$sort,$this->putVars['post'],
	...
	public function previewThread( $forumId )
	{
		...
		$return = array( 
  			...
  			dialogbuttons => array(
  				array( action => 'closedialog:previewthread', type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CLOSE'])
  			),  			
  		);
...
// TODO
	...
	public function delThread( $threadId )
	{
		...
		if (!$result) {
			...
			$return = $this->getForumInTable($forumId, true);
			$return ['dialogclose'] = array("delthread".$threadId, "thread".$threadId);
	...
	public function delThreadDialog( $threadId )
	{
		...
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['DEL_THREAD'], height => 200, modal => true )),
  			dialogid => 'delthread'.$threadId,
  			dialogbuttons => array(
  				array( action => 'delthread:'.$threadId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['DEL_THREAD']),
  				array( action => 'closedialog:delthread'.$threadId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),  			
  		);
  		return $return;
	...
	public function subscribeSubforumDialog($forumId) {
		...
		if ($subscriptionId) {
			...
			$dialogData .= '<div>' . '<input id="c4gForumSubscriptionForumOnlyThreads"  type="checkbox" name="subscription_only_threads" class="formdata"></input>' . '<label for="c4gForumSubscriptionForumOnlyThreads">' . 
	...
	public function moveThreadDialog( $threadId )
	{
		...
		if ($entries) {
			$data = $select;
			$dialogbuttons[] =
  					array( action => 'movethread:'.$threadId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['MOVE_THREAD']);
		} else {
			$data = $GLOBALS['TL_LANG']['C4G_FORUM']['MOVE_THREAD_NO_FORUMS'];
		}
		$dialogbuttons[] =
  				array( action => 'closedialog:movethread'.$threadId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL']);
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['MOVE_THREAD'], height => 200, modal => true )),
	...
	public function addMemberDialog( $forumId )
	{
		...
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['ADD_MEMBER'], height => 200, modal => true )),
  			dialogid => 'addmember'.$forumId,
  			dialogstate => "forum:".$forumId.";addmemberdialog:".$forumId,				
			dialogbuttons => array(
  				array( action => 'addmember:'.$forumId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['ADD_MEMBER']),
  				array( action => 'closedialog:addmember'.$forumId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),  			
  		);
  		return $return;
	...
	public function delPost( $postId )
	{
		...
		if (!$result) {
			...
			$return ['dialogclose'] = array("delpost".$postId, "post".$postId);
	...
	public function delPostDialog( $postId )
	{
		...
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['DEL_POST'], height => 300, modal => true )),
  			dialogid => 'delpost'.$postId,
  			dialogbuttons => array(
  				array( action => 'delpost:'.$postId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['DEL_POST']),
  				array( action => 'closedialog:delpost'.$postId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),  			
  		);
  		return $return;
	...
	public function editPost( $postId )
	{
		...
		$this->putVars['osmId'] = $this->putVars['osmIdType'] . '.' . $this->putVars['osmId'];
		$result = $this->helper->updatePostDB($post, $this->User->id, $this->putVars['subject'],$this->putVars['post'],
			$this->putVars['linkname'], $this->putVars['linkurl'], $this->putVars['geox'],$this->putVars['geoy'],
			$this->putVars['locstyle'], $this->putVars['label'], $this->putVars['tooltip'], $this->putVars['geodata'], $this->putVars['osmId']);			
		...
		if (!$result) {
			...
			$return = $this->getForumInTable($post['forumid'], true);
			$return ['dialogclose'] = array("editpost".$postId, "post".$postId);
			if ($this->c4g_forum_threadclick=='THREAD') {
				$return ['performaction'] = "readthread:".$post['threadid'];
			} else  {	
	...
	public function editThread( $threadId )
	{
		...
		if (!$result) {
			...
			$return ['dialogclose'] = array("editthread".$threadId, "thread".$threadId);
			$return ['performaction'] = "readthread:".$threadId;
	...
	{
		if ($this->helper->checkPermissionForAction($forumid,'postlink')) {
			$addClass = "";
	...
	public function getPostMapEntryForForm( $divname, $forumId, $dialogId, $geox, $geoy, $geodata, $locstyle, $label, $tooltip, $postId, $osmId )
	{
		if ($this->map_enabled()) {
			...
			if (($forum['enable_maps']) || ($forum['enable_maps_inherited'])){ 
				...
				// OSM-ID(-Picker)
					//check Permission
					if (!$this->helper->checkPermission($forumId,'mapextend')) { return ''; }
...
//			 			'<input name="locstyle" id="'.$dialogId.'_locstyle" value="'.$locstyle.'" type="hidden" class="formdata">'.
				...
				// GEO-Picker & Editor
					...
					//check Permission
					if (!$this->helper->checkPermission($forumId,'mapedit')) { return ''; }
	...
	public function editPostDialog( $postId )
	{
		...
		$data = '<div class="c4gForumEditPost">'.
				...
				$GLOBALS['TL_LANG']['C4G_FORUM']['SUBJECT'].':<br/>'.
				'<input name="subject" value="'.$post['subject'].'" type="text" class="formdata ui-corner-all" size="80" maxlength="100"></input><br />'.
				'</div>'.
				'<div class="c4gForumEditPostContent">'.
				$GLOBALS['TL_LANG']['C4G_FORUM']['POST'].':<br/>'.
				'<input type="hidden" name="uploadEnv" value="{{env::path}}">'.
				'<input type="hidden" name="uploadPath" value="'.$this->c4g_forum_bbcodes_editor_imguploadpath.'">'.
		...
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['EDIT_POST'], modal => true )),
  			dialogid => $dialogId,
			dialogstate => "forum:".$post['forumid'].";editpostdialog:".$postId,
			dialogbuttons => array(
  				array( action => 'editpost:'.$postId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SAVE_POST_CHANGES']),
  				array( action => $previewAction.':'.$postId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['PREVIEW']),
  				...
  				array( action => 'cancelpost:'.$post['threadid'].':'.$dialogId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),  			
  		);
  		return $return;
	...
	public function editThreadDialog( $threadId )
	{
		...
		$data = '<div class="c4gForumEditThread">'.
				...
				$GLOBALS['TL_LANG']['C4G_FORUM']['THREAD'].':<br/>'.
				'<input name="thread" value="'.$thread['name'].'" type="text" class="formdata ui-corner-all" size="80" maxlength="100"></input><br />'.						
		$data .= '</div>';
		...
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['EDIT_THREAD'], modal => true )),
  			dialogid => $dialogId,
  			dialogstate => "forum:".$thread['forumid'].";editthreaddialog:".$threadId,				
			dialogbuttons => array(
  				array( action => 'editthread:'.$threadId, type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SAVE_THREAD_CHANGES']),
  				array( action => 'closedialog:'.$dialogId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),  			
  		);
  		return $return;
	...
	public function postLink( $forumId, $dialogId )
	{
		...
		$data = '<div class="c4gForumPostLink">'.
				...
				'<input name="linkname" value="" '.
					...
					'data-target="'.$linkName.'" data-trgattr="value" type="text" class="formlink ui-corner-all" size="80" maxlength="80"></input><br />'.
				$GLOBALS['TL_LANG']['C4G_FORUM']['LINKURL'].':<br/>'.
				'<input name="linkurl" value="" '.
					'data-source="'.$linkUrl.'" data-srcattr="value" '.
					'data-target="'.$linkUrl.'" data-trgattr="value" type="text" class="formlink ui-corner-all" size="80" maxlength="255"></input><br />'.
				'</div>';
		...
		$return = array( 
  			dialogtype => "html", 
  			dialogdata => $data,
  			dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['EDIT_POST_LINK'], modal => true, height => 300 )),
  			dialogid => 'postlink'.$forumId,
  			dialogbuttons => array(
  				array( action => 'submit', type => 'submit', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SUBMIT']),
  				array( action => 'clear', type => 'submit', text => $GLOBALS['TL_LANG']['C4G_FORUM']['DELETE_LINK']),
  				array( action => 'closedialog:postlink'.$forumId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
  			),  			
  		);
  		return $return;
	...
	public function postMapEntry( $forumId, $dialogId, $add, $postId )
	{
		...
		if ($forum['map_type']=='PICK') {
			...
			$data .='<div class="c4gForumPostMapGeoCoords">'.
					...
					'data-target="'.$geox.'" data-trgattr="value" disabled type="text" class="formlink ui-corner-all" size="20" maxlength="20"></input>'.
					'<input id="c4gForumPostMapEntryGeoY" name="geoy" value="" '.
					'data-source="'.$geoy.'" data-srcattr="value" '.
					'data-target="'.$geoy.'" data-trgattr="value" disabled type="text" class="formlink ui-corner-all" size="20" maxlength="20"></input>'.
					'</div>';
		}
		else {
			// Feature Editor
		...
		if ($forum['map_label'] == 'CUST') {
			$data .=
				...
				'data-target="'.$label.'" data-trgattr="value" type="text" class="formlink ui-corner-all" size="50" maxlength="100"></input>'.
				'</div>';
		}
		...
		if ($forum['map_tooltip'] == 'CUST') {
			$data .=
				...
				'data-target="'.$tooltip.'" data-trgattr="value"  type="text" class="formlink ui-corner-all" size="50" maxlength="100"></input>'.
				'</div>';
		}
		$data .=
				'</div>';
		...
		$return = array(
				...
				dialogdata => $data,
				mapdata => $mapData,
				dialogoptions => $this->addDefaultDialogOptions(array( title => 
						sprintf($title,
							($forum['map_location_label'] ? $forum['map_location_label'] : $GLOBALS['TL_LANG']['C4G_FORUM']['LOCATION'])								
						)
					, modal => true	
		...
		$return['dialogbuttons'][] = array( action => 'submit', type => 'submit', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SUBMIT']);
		if (!$add) {
			$return['dialogbuttons'][] =
				array( action => 'clear', type => 'submit', text =>
		...
		$return['dialogbuttons'][] = array( action => 'closedialog:postmapentry'.$forumId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL']);		
		return $return;
	...
	public function viewMapForPost( $postId )
	{
		...
		$return = array(
				...
				dialogdata => $data,
				mapdata => $mapData,
				dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['VIEW_MAP_FOR_POST'], modal => true )),
				dialogid => 'viewmapforpost'.$postId,
  				dialogstate => "forum:".$post['forumid'].";viewmapforpost:".$postId,				
				dialogbuttons => array(
						array( action => 'closedialog:viewmapforpost'.$postId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CLOSE'])
				),
		);
		return $return;
	...
	public function viewMapForForum( $forumId )
	{
		...
		$return = array(
				...
				dialogdata => $data,
				mapdata => $mapData,
				dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['VIEW_MAP_FOR_FORUM'], modal => true )),
				dialogid => 'viewmapforforum'.$forumId,
  				dialogstate => "forum:".$forumId.";viewmapforforum:".$forumId,				
				dialogbuttons => array(
						array( action => 'closedialog:viewmapforforum'.$forumId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CLOSE'])
				),
		);
		return $return;
	...
	public function searchDialog( $forumId )
	{
		...
		$return = array(
				dialogtype => "html",
				dialogdata => $data,
				dialogoptions => $this->addDefaultDialogOptions(array( title => $GLOBALS['TL_LANG']['C4G_FORUM']['SEARCHDIALOG_HEADLINE'], modal => true, width => 470, height => 325)),
				dialogid => $dialogId,
				dialogstate => $action.";searchDialog:".$forumId,
				dialogbuttons => array(
						array( action => 'search:'.$forumId, 'class' => 'c4gGuiDefaultAction', type => 'send', text => $GLOBALS['TL_LANG']['C4G_FORUM']['SEARCH']),
						array( action => 'closedialog:'.$dialogId, type => 'get', text => $GLOBALS['TL_LANG']['C4G_FORUM']['CANCEL'])
				),
		);
		return $return;
	...
	public function search($forumId, $searchParam)
	{
		...
		$data['aoColumnDefs'] = array(
				array('sTitle' => 'key', bVisible => false, bSearchable => false, aTargets => array(0)),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['THREAD'], sClass => 'c4g_forum_searchres_threadname', sWidth => '30%', aDataSort => array(10,1), aTargets => array(1), c4gMinTableSizeWidths => array(array(tsize=>500,width=>'50%'),array(tsize=>0,width=>''))),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['SEARCHRESULTPAGE_DATATABLE_AREA'], sClass => 'c4g_forum_searchres_area', sWidth => '20%', aTargets=>array(2)),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['LAST_AUTHOR_SHORT'], sClass => 'c4g_forum_searchres_last_author', aDataSort => array(10,3,5), bSearchable => false, aTargets => array(3), c4gMinTableSize => 700),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['LAST_POST_SHORT'], sClass => 'c4g_forum_searchres_last_post',aDataSort => array(11,5), bSearchable => false, asSorting => array( 'desc', 'asc' ), aTargets => array(4), c4gMinTableSize => 700),
				array(bVisible=>false, bSearchable => false, aTargets=>array(5)  ),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['AUTHOR'], sClass => 'c4g_forum_searchres_author', aDataSort => array(10,6,8), bSearchable => false, aTargets => array(6), c4gMinTableSize => 500),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['CREATED_ON'], sClass => 'c4g_forum_searchres_created', aDataSort => array(11,8), asSorting => array( 'desc', 'asc' ), bSearchable => false, aTargets => array(7), c4gMinTableSize => 500),
				array(bVisible=>false, bSearchable => false, aTargets=>array(8)),
				array('sTitle' => '#', sClass => 'c4g_forum_searchres_postcount', asSorting => array( 'desc', 'asc' ), bSearchable => false, aTargets => array(9)),
				array(bVisible=>false, bSearchable => false, aTargets=>array(10)),
				array(bVisible=>false, bSearchable => false, aTargets=>array(11)),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['SEARCHRESULTPAGE_DATATABLE_HITS'], sClass => 'c4g_forum_searchres_hits', bVisible => true, bSearchable => false, aTargets => array(12)),
				array(bVisible=>false, bSearchable => false, aTargets=>array(13)),
		);
		if ($this->c4g_forum_table_jqui_layout) {
			$data['bJQueryUI'] = true;
		}
		
		$data['aaSorting'] = array(array(12, 'desc'));
		$data['bScrollCollapse'] = true;
		$data['bStateSave'] = true;
		$data['sPaginationType'] = 'full_numbers';
		$data['oLanguage'] = array(
		...
		$return = array(
				...
				buttons => array(array( id=>'searchDialog:'.$forumId, text=>$GLOBALS['TL_LANG']['C4G_FORUM']['SEARCHRESULTPAGE_BUTTON_START_NEW_SEARCH']))
		);
		return $return;
	}
	...
	public function getThreadlist($forumId)
	{
		...
		$data['aoColumnDefs'] = array(
				array('sTitle' => 'key', bVisible => false, bSearchable => false, aTargets => array(0)),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['THREAD'], sWidth => '30%', aDataSort => array(1), aTargets => array(1), c4gMinTableSizeWidths => array(array(tsize=>500,width=>'50%'),array(tsize=>0,width=>''))),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['SEARCHRESULTPAGE_DATATABLE_AREA'], sWidth => '20%', aTargets=>array(2)),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['LAST_AUTHOR_SHORT'], aDataSort => array(3,5), bSearchable => false, aTargets => array(3), c4gMinTableSize => 700),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['LAST_POST_SHORT'], aDataSort => array(5), bSearchable => false, asSorting => array( 'desc', 'asc' ), aTargets => array(4), c4gMinTableSize => 700),
				array(bVisible=>false, bSearchable => false, aTargets=>array(5)  ),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['AUTHOR'], aDataSort => array(6,8), bSearchable => false, aTargets => array(6), c4gMinTableSize => 500),
				array('sTitle' => $GLOBALS['TL_LANG']['C4G_FORUM']['CREATED_ON'], aDataSort => array(8), asSorting => array( 'desc', 'asc' ), bSearchable => false, aTargets => array(7), c4gMinTableSize => 500),
				array(bVisible=>false, bSearchable => false, aTargets=>array(8)),
				array('sTitle' => '#', asSorting => array( 'desc', 'asc' ), bSearchable => false, aTargets => array(9)),
				array(bVisible=>false, bSearchable => false, aTargets=>array(10)),
		);
		if ($this->c4g_forum_table_jqui_layout) {
			$data['bJQueryUI'] = true;
		...
		$data['aaSorting'] = array(array(4, 'desc'));
		$data['bScrollCollapse'] = true;
		$data['bStateSave'] = true;
		$data['sPaginationType'] = 'full_numbers';
		$data['oLanguage'] = array(
		...
		$return = array(
				...
				buttons => array()//array(array( id=>'threadlist:'.$forumId, text=>$GLOBALS['TL_LANG']['C4G_FORUM']['LATESTTHREADS']))
		);
		return $return;
	}
	...
	public function addDefaultButtons($buttons,$forumId) 
	{
		...
		//}	
		if($this->helper->checkPermissionForAction($forumId, 'search')){
			$buttons[] = array( id=>'searchDialog:'.$forumId, text=>$GLOBALS['TL_LANG']['C4G_FORUM']['SEARCH']);
		}
		
		if($this->helper->checkPermissionForAction($forumId, 'latestthreads') && ($this->action == "forumbox")){		
		  $buttons[] = array( id=>'threadlist:'.$forumId, text=>$GLOBALS['TL_LANG']['C4G_FORUM']['LATESTTHREADS']);
		}
	...
	{	
		...
		foreach ($path as $value) {
			...
			if (($value['use_intropage']) && (!$this->c4g_forum_hide_intropages)){
				$data[] = array( id=>'forumintro:'.$value['id'],text=>$value['name']);
			} else {								
				if ($value['subforums']==0) {
					$data[] = array( id=>'forum:'.$value['id'],text=>$value['name']);							
				} else {
					$data[] = array( id=>'forumbox:'.$value['id'],text=>$value['name']);											
				}
			}
		}
		return $data;
	...
	protected function compile()
	{
		...
		if ($this->c4g_forum_uitheme_css_src) {
			...
			} else if (is_numeric($this->c4g_forum_uitheme_css_src))
			{
				// Contao 3.x Format 
